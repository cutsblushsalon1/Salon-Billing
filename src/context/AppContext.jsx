...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // Automatic expenses are generated by the idempotent fresh-read routine
  ...
  // Create scheduled monthly salary/rent expenses idempotently. Each generated
  // expense has a deterministic recurringKey, so reopening the app or another
  // device cannot create duplicates for the same month.
  const ensureAutomaticMonthlyExpenses = useCallback(async () => {
    const today = new Date()
    const year = today.getFullYear()
    const month = today.getMonth()
    const monthKey = `${year}-${String(month + 1).padStart(2, '0')}`
    const requestedDay = (value) => Math.min(31, Math.max(1, Number(value) || 1))
    const clampDay = (value) => Math.min(requestedDay(value), new Date(year, month + 1, 0).getDate())
    const generated = []
    ...
    if (settings.autoRentExpenseEnabled && Number(settings.autoRentAmount) > 0 && today.getDate() >= clampDay(settings.autoRentExpenseDay)) {
      generated.push({
        id: `auto_rent_${monthKey}`,
        recurringKey: `rent:${monthKey}`,
        date: `${year}-${String(month + 1).padStart(2, '0')}-${String(clampDay(settings.autoRentExpenseDay)).padStart(2, '0')}T12:00:00`,
        category: 'Rent',
        description: 'Monthly rent',
        amount: Number(settings.autoRentAmount),
        paymentMethod: 'Bank Transfer',
        notes: 'Automatic monthly rent expense',
        source: 'automatic',
        createdAt: today.toISOString(),
      })
    }
  ...
  useEffect(() => {
    if (!hydrated || !isSupabaseConfigured || !settings.autoExpensesSavedAt) return
    ensureAutomaticMonthlyExpenses()
  }, [hydrated, ensureAutomaticMonthlyExpenses, settings.autoExpensesSavedAt])
