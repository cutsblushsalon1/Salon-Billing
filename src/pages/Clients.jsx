...
import { Search, UserPlus, Users, Phone, Receipt, ArrowUpRight, Pencil, Trash2, Upload, Check } from 'lucide-react'
...
const HEADER_ALIASES = {
  ...
  totalSpent: ['totalspent', 'total spent', 'lifetime spend', 'total spend'],
}
...
const emptyForm = { name: '', phone: '', email: '', gender: 'Female', notes: '' }
...
export default function Clients() {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        results.data.forEach((row) => {
          ...
          const existing = record.phone ? clients.find((c) => c.phone === record.phone) : null
          upsertClient({
            ...
            notes: record.notes || '',
            totalSpent: record.totalSpent ? Number(record.totalSpent) || 0 : existing?.totalSpent || 0,
          })
  ...
  const filtered = useMemo(() => {
    ...
    return clients
      ...
      .sort((a, b) => new Date(b.lastVisit || b.createdAt) - new Date(a.lastVisit || a.createdAt))
  }, [clients, query])
  ...
  function openEdit(c) {
    setForm({ name: c.name, phone: c.phone || '', email: c.email || '', gender: c.gender || 'Female', notes: c.notes || '' })
    setEditingId(c.id)
    setModalOpen(true)
  }
  ...
  return (
    <div>
      ...
      <div className="relative mb-6 max-w-md">
        ...
        <input className="input pl-10" placeholder="Search by name or phone…" value={query} onChange={(e) => setQuery(e.target.value)} />
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          subtitle={clients.length === 0 ? 'Add your first client to start tracking visits and spend.' : 'Try a different search term.'}
          action={
            clients.length === 0 && (
              <button className="btn-primary" onClick={openAdd}>
                <UserPlus size={16} /> Add Client
            ...
            <div key={c.id} className="card p-5 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
                <div className="bg-black/[0.02] rounded-lg p-2.5">
                  ...
                  <p className="font-semibold text-ink tabular">{formatCurrency(c.totalSpent || 0, settings.currencySymbol)}</p>
                </div>
                <div className="bg-black/[0.02] rounded-lg p-2.5">
                  <p className="text-muted mb-0.5">Visits</p>
                  <p className="font-semibold text-ink tabular">{c.visits?.length || 0}</p>
                </div>
              </div>

              <p className="text-xs text-muted mb-4">
                {c.lastVisit ? `Last visit ${formatDate(c.lastVisit)}` : `Added ${formatDate(c.createdAt)}`}
              </p>

              <div className="flex items-center gap-2 mt-auto pt-3 border-t border-black/5">
                <button onClick={() => navigate(`/clients/${c.id}`)} className="btn-ghost text-xs py-1.5 flex-1">
                  Profile <ArrowUpRight size={13} />
                </button>
                <button
                  onClick={() => navigate('/new-bill', { state: { clientId: c.id } })}
                  className="btn-brass text-xs py-1.5 flex-1"
                >
                  <Receipt size={13} /> Bill
                </button>
                <button onClick={() => openEdit(c)} className="p-2 text-muted hover:text-plum">
                  <Pencil size={14} />
                </button>
                <button onClick={() => setConfirmDelete(c)} className="p-2 text-muted hover:text-danger">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
        ...
        {importResult && (
          <div>
            ...
            <ul className="text-sm text-ink space-y-1.5 mb-5">
              <li>{importResult.added} new client{importResult.added === 1 ? '' : 's'} added</li>
              <li>{importResult.updated} existing client{importResult.updated === 1 ? '' : 's'} updated (matched by phone)</li>
              {importResult.skipped > 0 && <li className="text-muted">{importResult.skipped} row{importResult.skipped === 1 ? '' : 's'} skipped (missing name)</li>}
            </ul>
      ...
      <Modal open={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Remove client?" size="sm">
        {confirmDelete && (
          <div>
            <p className="text-sm text-muted mb-5">
              This removes <span className="font-semibold text-ink">{confirmDelete.name}</span> from your directory. Past bills stay in
              billing history.
            </p>
